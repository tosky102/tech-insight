...
export const api = {
   ...
   */
  getArticles: (params: { page?: number; page_size?: number; keyword?: string; sort?: string }) => {
    const sp = new URLSearchParams();
    if (params.page != null) sp.set('page', String(params.page));
    if (params.page_size != null) sp.set('page_size', String(params.page_size));
    if (params.keyword) sp.set('keyword', params.keyword);
    if (params.sort) sp.set('sort', params.sort);
    return request<import('@/types/article').ArticleListResponse>(`/articles?${sp}`);
  },
  /**
   * セマンティック検索（AI 検索）
   *
   ...
   */
  searchSemantic: (q: string, params?: { page?: number; page_size?: number }) => {
    const sp = new URLSearchParams({ q });
    if (params?.page != null) sp.set('page', String(params.page));
    if (params?.page_size != null) sp.set('page_size', String(params.page_size ?? 20));
    return request<import('@/types/article').ArticleListResponse>(`/articles/search/semantic?${sp}`);
  },
  /** 単一記事の取得 */
  ...
  createArticle: (data: import('@/types/article').ArticleCreateInput) =>
    request<import('@/types/article').Article>('/articles', { method: 'POST', body: JSON.stringify(data) }),
  /** 記事の更新（差分のみ送信可能） */
  updateArticle: (id: number, data: import('@/types/article').ArticleUpdateInput) =>
    request<import('@/types/article').Article>(`/articles/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  /** 記事の削除 */
